...
Page({
  ...
  specPrice: function () {
    ...
    if (this.complete()) {
      ...
      for (i = 0; i < len; i++) {
        if (sku[i].spec == name) {
          if (sku[i].stock > 0) {
            goodData.sku_price = sku[i].integral_price
  ...
  buyGood: function () {
    ...
    app.judgelogin({
      success(res) {
        wx.navigateTo({
          url: "/pages/integShop/confirmOrder/confirmOrder?skus=" + that.data.goodData.id + '&num=' + that.data.buyNum + '&quick=' + 1
        })
      }
    })
  },
    ...
    app.wxrequest({
      ...
      success: function (res) {
        ...
        if (res.goods_sku.length == 1) {
          goodData.sku_price = res.goods_sku[0].integral_price
